...
from neural_methods.model.rwkv import Rwkv,RwkvConfig
from neural_methods.trainer.BaseTrainer import BaseTrainer
from tqdm import tqdm
# from neural_methods.trainer.balance import BalancedDataParallel


class RwkvTrainer(BaseTrainer):
    ...
    def __init__(self, config, data_loader):
        ...
        if config.TOOLBOX_MODE == "train_and_test":
            self.model = Rwkv(RwkvConfig(ctx_len=config.MODEL.RWKV.CTX_LEN,dropout=config.MODEL.RWKV.DROPOUT,
                rwkv_emb_scale=config.MODEL.RWKV.RWKV_EMB_SCALE, 
                n_layer=config.MODEL.RWKV.N_LAYER, n_head=config.MODEL.RWKV.N_HEAD, 
                n_embd=config.MODEL.RWKV.N_EMBD, 
                n_attn=config.MODEL.RWKV.N_ATTN, n_ffn=config.MODEL.RWKV.N_FFN)).to(self.device)
            # self.model = torch.nn.DataParallel(self.model, device_ids=list(range(config.NUM_OF_GPU_TRAIN)))
            # first_gpu_bsz = self.batch_size // (self.num_of_gpu*2)
            # self.model = BalancedDataParallel(first_gpu_bsz, self.model,  device_ids=list(range(config.NUM_OF_GPU_TRAIN)), dim=0).cuda()
    ...
    def train(self, data_loader):
        ...
        for epoch in range(self.max_epoch_num):
            ...
            for idx, batch in enumerate(tbar):
                ...
                # print(labels[0])
                # data = data // self.base_len * self.base_len
                # labels = labels // self.base_len * self.base_len
